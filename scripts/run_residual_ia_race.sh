...
#
# Invocation:
#   tmux new-session -d -s ria_p1 'cd ~/neuralips26 && bash scripts/run_residual_ia_race.sh phase1 2>&1 | tee results/ria_p1.log'
#   tmux new-session -d -s ria_p2 'cd ~/neuralips26 && bash scripts/run_residual_ia_race.sh phase2 256 2>&1 | tee results/ria_p2.log'
#
# Worker sharding (optional): bash scripts/run_residual_ia_race.sh phase1 worker 1 2
