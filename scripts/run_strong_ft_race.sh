#!/bin/bash
# Strong FT sweep on RACE VM: 3 datasets x 3 seeds x 10 configs = 90 runs
# Supports worker sharding: bash scripts/run_strong_ft_race.sh worker K N
#
# Usage:
#   bash scripts/run_strong_ft_race.sh              # run all 90
#   bash scripts/run_strong_ft_race.sh worker 1 3   # worker 1 of 3
set -e
