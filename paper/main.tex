...
\begin{abstract}
Adapting time series foundation models (TSFMs) to downstream forecasting tasks typically involves selecting from a small menu of adapter configurations---LoRA ranks, prediction heads, pooling strategies. We propose \textbf{Code Evolution}, a framework that searches over arbitrary PyTorch adapter modules instead, evolving dataset-specific adapter architectures through population-based search in an unbounded code space. On the standard LTSF benchmark with MOMENT as backbone, per-dataset evolved adapters outperform the best hand-designed baselines on \textbf{all 8 experiments} across 5 datasets, 3 seeds, and 2 forecast horizons, with an average improvement of \textbf{9.1\%} in test MSE while using \textbf{58\% fewer parameters}. A systematic factorial decomposition reveals that the shift from discrete configurations to code-level search is the dominant driver of improvement, and that evolutionary selection discovers novel architectural patterns---depthwise convolutions from mobile vision, learned feature attention from NLP, batch normalization gating from computer vision---that do not exist in any hand-designed adapter menu. These cross-domain architectural transfers suggest that the design space for TSFM adapters is far richer than currently explored, opening new directions for automated, backbone-agnostic adapter design across foundation model families.
\end{abstract}
...
\begin{enumerate}
\item \textbf{Code Evolution}, a framework for searching the unbounded space of PyTorch adapter programs for TSFM adaptation. Per-dataset evolution discovers dataset-specific optimal architectures in 30 minutes on a single GPU.
...
\subsection{Main Results: Per-Dataset Evolution}

Table~\ref{tab:main} shows that per-dataset evolved adapters beat the best hand-designed baseline on \textbf{all 8 experiments}---across 5 datasets, 3 seeds on ETTh1, and 2 forecast horizons.
...
\textbf{Why per-dataset evolution matters.} Our initial experiments tested adapters evolved under one protocol on a different evaluation setup, yielding mixed results. Running evolution directly on the target protocol and dataset produces consistent wins (8/8). This validates the core thesis: different datasets benefit from different adapter architectures, and automated search discovers them.
