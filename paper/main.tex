...
Table~\ref{tab:main} shows results on the held-out test set. Evolved adapters---whose architecture is discovered via search on the validation set---beat the best fixed adapter on \textbf{all 8 experiments} across 5 datasets, 3 seeds on ETTh1, and 2 forecast horizons.
...
\toprule
Dataset & Setting & Evolved MSE & Best Fixed Adapter & $\Delta$ & Evolved Params \\
\midrule
...
\end{axis}
\end{tikzpicture}
\caption{Evolved adapters (blue) vs.\ best fixed adapter architecture (red; Conv1d + Linear, 209K params) on standard LTSF benchmarks. Evolved adapters win on all 5 datasets, with the largest gain on Weather (--18.9\%).}
