...
**On significance.** The lasting contribution is the diagnosis and its predictor, which the reframing above sharpens. R(D) is a practical, training-free tool: computed from raw statistics before training, it predicts whether raw routing will help on a new dataset (ρ=-0.88, n=9), and where it will not (Traffic, R=0.14). It transfers beyond our six benchmarks and our router, since two distinct variants (SR-MoA and Residual-IA+) confirm it. The diagnosis, the R(D) predictor, and the raw-routing intervention are what we ask the reader to take away.
