...
**W6 (minor).** The router is 853 params for the reported K=5 model: a Conv1d gate (528) plus a Linear(64,K) head (325 at K=5), consistent across the main text, Fig 2, Table A.1, and the released code. That head scales with K, so over the expert-count sweep (Table I.3, up to K=10, head 650) the router reaches 1,000-1,200 at the high end; the Checklist's "~1.1K" reflects that larger-K count, not the reported K=5, and we will correct it to 853. On the SR-MoA figure, the improvement is over RR-MoA (SR-MoA beats RR-MoA on all six datasets, Fig. F.1), not the baselines. We will also standardize "fine-tuning" and recheck the flagged R(D) parenthesis.
