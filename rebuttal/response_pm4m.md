...
**W4 (heavy tuning; generalization beyond six).** On "heavy tuning": the 546-run sweep (Appendix P) was a one-time search for the recipe, not a per-dataset cost; the recipe is then fixed, and should not read as a SOTA search. The result is a win over DLinear (65:11), not parity. [NEW] Applied unchanged (gate-init b=-2, 5-epoch warmup, shared NLinear, validation early-stop) to three unseen datasets, Residual-IA+ matches or beats DLinear on 8/9 cells: Solar 3/3 (-9.5%), Traffic 3/3 (parity), Exchange 2/3, no collapse. A recipe that transfers to unseen data without re-tuning is difficult to attribute to per-dataset tuning, and it speaks directly to "generalization beyond these six is unclear."
