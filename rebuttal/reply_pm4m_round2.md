...
We thank the reviewer for the time this review took. Both points came from reading our rebuttal against the paper's own appendices, and both were right. On the first we stand corrected; on the second we ran the requested control and report every cell.
...
Residual-IA+ matches or beats the stronger anchor on four of six, mean gap −0.9%. ETTh1 is the only significant loss; the ETTh2 gap does not reach significance.

The margin holds where level anchoring is inert: on Electricity and Weather above, and on Solar, which the recipe never saw, where the anchors differ by 0.3% and Residual-IA+ wins all 20 cells at n=10 (−8.7% at H=96, −11.2% at H=192). On Exchange we agree: the −26.3% over the single-Linear anchor is level anchoring, and against NLinear the cell is parity at H=96 and behind at H=192, so we will report it as parity. On the backbone's contribution, the reviewer is right that this comparison cannot separate it from the raw branch; App. P.3 takes that up and reports it as dataset-dependent.

The revision will score against both anchors, lead with the count against the stronger, name the anchor accurately wherever it appears, including Figure P.1's caption, which calls the raw branch DLinear-equivalent, and move the anchor comparison and backbone-contribution analysis into the main text.

To be explicit: the confusion here comes from our writing, not the reviewer's reading. Both concerns focus on the accuracy claim rather than the diagnostic contribution because our own presentation put it in front: we led with "5/6 match-or-beat" and titled the appendix "Closing the DLinear Gap". The reviewer read the paper as we wrote it. That said, we would respectfully ask that the paper also be weighed on the diagnostic contribution, which stands as submitted; the title and two of three contributions name it.
