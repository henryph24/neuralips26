...
Given that the memory, expert-scaling, normalizer-generality, task-diversity, convergence, and statistical concerns are now addressed (several already in the submission; full per-cell tables will follow in the revision if accepted), we would be grateful if the reviewer would reconsider the score.
