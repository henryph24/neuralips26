...
**On significance.** We appreciate that the review recognizes the signal-ratio's value ("predicting dataset vulnerability prior to training... practical value beyond the specific method"). This is exactly the value the reviewer highlights: R(D) is training-free and computed a priori (ρ=-0.88, n=9), so a practitioner can decide before training whether raw routing helps, and where it will not (Traffic, R=0.14). With the collapse diagnosis, that is a reusable result for the TSFM-adapter community, independent of which backbone wins. It is also general, not LTSF-specific: it shows up under BatchNorm, GroupNorm, and in a vision model (App. H, H.1), extending to any instance-normalized backbone with a mixture head.
