...
We sincerely thank the reviewer for the careful assessment, and are glad the causal mechanism and the signal-ratio metric came through as valuable. Several points are already addressed by experiments in the paper, which we will highlight in the revision (memory, expert scaling, normalizers, raw routing on an existing MoE, a vision replication); where a concern needed something new (a LayerNorm swap, 200-epoch convergence, a significance re-analysis), we ran it and tag it [NEW].

**W1 (memory of raw routing).** *(Meta-review point 5.)* We can understand why routing on raw inputs sounds memory-heavy; in practice it is the opposite. The router reads a single univariate window (B×L, ~0.26 MB), ~64× smaller than the hidden states a conventional router reads (B×T×d, ~17 MB at batch 128); the window is the model's own input, already resident, a re-read, not a new allocation. It is an 853-parameter Conv1d gate (0.002% of the ~40M backbone), adding under 0.1 ms to the 52.9 ms forward pass. Peak memory is set by the frozen backbone every adapter already loads: the three variants add +5.9% latency and at most +6 MB on a 359 MB backbone (Table B.1). So raw routing adds no meaningful memory, and the reviewer was right to ask us to show it, which the revision will.

**W2 (dataset-distributional / feature-asymmetry factors).** *(Meta-review point 6: theory limited to IN–router interaction.)* We appreciate this, and the theory reaches well beyond the IN–router interaction. Observation 1 is a four-part mutual-information decomposition: an exact identity, a bound, and a data-processing corner where normalization destroys the entire routing distribution (I(S;E)=0). Appendix D.1 adds a rate-distortion reading (instance normalization is lossy compression, R(D) its rate-distortion slope) and Proposition 2, which pins the phase transition: an SNR onset at α*=R(D)/(R(D)+1) and a softmax collapse at α=1. Proposition 1 is a separate, dynamical result: gradient co-adaptation drives the entropy collapse over training, confirmed in the full 8-block Transformer. R(D) is a falsifiable predictor: it predicts vulnerability before training (ρ=-0.88, n=9), is right on the boundary cases (Traffic R=0.14, Solar R=0.06), and rules out a generic SNR account. It also holds beyond instance normalization, under BatchNorm1d, GroupNorm, and in a vision model (App. H, H.1, W4). What is genuinely open is formalizing dataset-specific structure and feature asymmetries beyond (μ,σ); our ablations already show the router uses more than (μ,σ) (App. F, SSR +48%).

**W3 (scaling experts → router-input dimensionality / bandwidth).** *(Meta-review point 7.)* The concern is fair for a standard MoE, where the router input grows with the expert count. In ours it does not, for architectural reasons: the router input is a fixed-size raw window, the same at K=5 and K=20, and Top-2 routing runs two experts per sample at any K (2 of 5 at K=5, 2 of 20 at K=20), so active compute and bandwidth do not grow with K; only the tiny K-way logit layer (64→K) grows. Table I.3 (K∈{1,2,3,5,7,10}) confirms it: stable for K≥2, so expert count creates no router-input or bandwidth bottleneck. [NEW] Extending the sweep to K=15 and K=20 (ETTh1, 3 seeds): MSE stays stable (0.67-0.73), entropy 91-97% of its log K ceiling (no collapse), and RR-MoA still beats the best fixed adapter by ~40% at every K. Beyond pool size, we varied the composition (richer conv/residual/gated, deeper MLP, a ten-way pool) and took a first step on learned pools (a hypernetwork generates each expert); the tables below show routing stays healthy and beats the best fixed adapter, with a rigorous learned-pool method (larger generators, NAS) left to future work.
...
**W4 (other normalizers: BatchNorm / LayerNorm / RMSNorm).** *(Meta-review point 8: causal intervention beyond IN.)* This is a natural question, and the submitted App. H already tests it: swapping RevIN for BatchNorm1d (entropy 0.62→0.004) and GroupNorm (0.51→0.000) yields identical collapse, while removing normalization keeps it healthy (0.82). [NEW] For LayerNorm, we swap one into MOMENT's input-normalization position (same App. H protocol): it collapses identically (entropy → 0.000), no-norm control healthy at 0.825. The tension with our LayerNorm/RMSNorm negative controls (Chronos, Timer-XL) comes down to where the norm sits: at the input position it strips per-window statistics and collapses, while encoder-internal LayerNorm/RMSNorm do not strip the router's input, so they do not collapse. The rule is whether the normalizer strips routing-relevant statistics at the router's input, so the mechanism is not IN-specific.

**W5 (only forecasting / imputation).** Beyond forecasting on six datasets and imputation (Table J.1, 7/8 wins), [NEW] a classification check on three UEA datasets keeps entropy healthy (1.46-1.59), so the mechanism is not forecasting-specific (frozen features cap accuracy). [NEW] An input-length ablation (L∈{96,192,336,512}, 72 runs) shows no collapse at any L, raw beating normalized routing at all 24 cells. [NEW] A reconstruction-based anomaly check on SMD adds a fourth task type: entropy stays healthy (1.52-1.61, no collapse), detection frozen-capped (ROC-AUC ~0.55). So the mechanism holds across four task types; generative adaptation is a natural next step.

**W6 / Q3 (few epochs / convergence).** We understand why 15 epochs may seem few. [NEW] Trained to 200 epochs on ETTh1, ETTm1, Weather and Electricity (the slowest to converge; 3 seeds each), RR-MoA never collapses (seed-mean entropy 1.02-1.34, min 0.87 across 12 runs) and its MSE is stable-to-improved versus 15 epochs (ETTh1 0.680→0.646, ETTm1 0.564→0.493), so the reported numbers are conservative. Extended full fine-tuning (50-100 epochs) still loses to 15-epoch frozen RR-MoA by 33-71% (App. O); the dual-stream Residual-IA+ overfits by 50 epochs, so we report its 15-epoch result.
...
**On significance.** We appreciate that the review recognizes the signal-ratio's value ("predicting dataset vulnerability prior to training... practical value beyond the specific method"). This is the paper's most broadly useful contribution: R(D) is training-free and computed a priori (ρ=-0.88, n=9), so a practitioner can decide before training whether raw routing helps, and where it will not (Traffic, R=0.14). With the collapse diagnosis, that is a reusable result for the TSFM-adapter community, independent of which backbone wins. It is also general, not LTSF-specific: it shows up under BatchNorm, GroupNorm, and in a vision model (App. H, H.1), extending to any instance-normalized backbone with a mixture head.
