...
**W2 (dataset-distributional / feature-asymmetry factors).** *(Meta-review point 6: theory limited to IN–router interaction.)* The theory is deeper than the IN–router interaction alone. Observation 1 is a four-part mutual-information decomposition: an exact identity, a quantitative bound, and a data-processing corner where normalization destroys the entire routing distribution (I(S;E)=0). Appendix D.1 adds two layers: a rate-distortion reading in which instance normalization is lossy compression and R(D) is its rate-distortion slope (the routing bits it strips), and Proposition 2, which pins the phase transition, a signal-to-noise onset at α*=R(D)/(R(D)+1) and a softmax collapse at α=1. Proposition 1 is a separate, dynamical result: gradient co-adaptation drives the entropy collapse over training, confirmed in the full 8-block Transformer. R(D) is falsifiable, not curve-fit: it predicts vulnerability before training (ρ=-0.88, n=9), is correct on the boundary cases (Traffic R=0.14, Solar R=0.06), and rules out a generic signal-to-noise account. The theory also holds beyond instance normalization, under BatchNorm1d, GroupNorm, and in a vision model (App. H, H.1, W4). What is genuinely open is a full formalization of dataset-specific structure and feature asymmetries beyond (μ,σ); our ablations already show the router uses more than (μ,σ) (App. F, SSR +48%).
