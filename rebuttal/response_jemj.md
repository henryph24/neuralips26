...
So raw routing stays healthy on both new task types, and the anti-collapse mechanism carries over beyond forecasting.

**Frozen-backbone requirement / Q2 (raw routing + selective unfreezing).** *(Meta-review point 4: "the method requires a strictly frozen backbone.")* A strictly frozen backbone is not required. Raw routing combines with selective unfreezing, and it works well: Table 1 shows RR-MoA (raw routing) beating the best fixed adapter not only frozen but also with Last-2 and Last-4 blocks unfrozen (ETTh1 -44% / -29% / -32%). Frozen is simply the strongest setting on most datasets (4/6), which Proposition 1 explains via a gradient co-adaptation feedback loop. It is also a deployment advantage rather than a constraint: because the backbone stays fixed, one shared TSFM can serve many tasks through small hot-swappable adapters (App. B), which makes the multi-tenant setup practical. So a strictly frozen backbone is a design choice the method benefits from, not a requirement it imposes.
