...
\node[font=\tiny, text=red!70!black] at (axis cs:ETTh1,1.21) {$-36\%$};
\node[font=\tiny, text=red!70!black] at (axis cs:ETTm1,1.00) {$-35\%$};
\node[font=\tiny, text=red!70!black] at (axis cs:Weath,0.58) {$-41\%$};
\node[font=\tiny, text=red!70!black] at (axis cs:ETTh2,2.79) {$-71\%$};
\node[font=\tiny, text=red!70!black] at (axis cs:ETTm2,2.80) {$-79\%$};
\node[font=\tiny, text=red!70!black] at (axis cs:Elec,0.57) {$-12\%$};
\legend{Full fine-tuning (all unfrozen), Frozen RR-MoA (Top-2)}
\end{axis}
\end{tikzpicture}
\caption{\textbf{The Frozen Paradox.} Frozen RR-MoA (blue) beats full backbone fine-tuning with all 8 encoder layers unfrozen (red) by $12$--$79\%$ across all 6 datasets on MOMENT-small, reversing the standard PEFT heuristic that unfreezing improves adaptation. Error bars: std over 3 seeds (full-FT) / 5 seeds (RR-MoA).}
...
\textbf{Cross-backbone generalization.}\label{sec:cross_backbone} We evaluate RR-MoA on four architecturally distinct backbones. The results reveal a \emph{normalization spectrum}: the benefit of raw routing scales with how aggressively the backbone's internal normalization degrades sample-discriminative statistics.

\textit{MOMENT-small/large} (RevIN + 8/24 LayerNorm blocks): RR-MoA wins \textbf{all 18 configurations} per backbone (6 datasets $\times$ 3 seeds), with improvements of $27$--$81\%$. RevIN is the primary mechanism: it explicitly strips per-window mean and scale before the encoder.

\textit{Moirai} (no RevIN, 6 LayerNorm blocks): RR-MoA wins \textbf{18/18} with improvements of $3$--$31\%$ (full table in Appendix~\ref{app:cross_backbone}). Even without RevIN, repeated LayerNorm progressively homogenizes hidden states, making raw routing valuable. Moirai+RR-MoA on Weather reaches $0.209{\pm}0.004$---matching DLinear from scratch ($0.208$).

\textit{Chronos-T5-small} (no RevIN, encoder-decoder): RR-MoA wins only \textbf{1/9}. Chronos's T5 encoder-decoder architecture preserves more sample-level information in hidden states, and the conv baseline already achieves strong MSE ($0.659$ on ETTh1).\footnote{Chronos results use a generic patch-and-pad input projection rather than Chronos's learned tokenizer; some performance gap may reflect integration quality.} AdaMix on Chronos retains entropy $1.48$--$1.60$ under unfreezing (no collapse), independently confirming that routing collapse is normalization-specific.

The gradient of improvement---$81\%$ (MOMENT+RevIN) $>$ $31\%$ (Moirai, LayerNorm only) $>$ ${\sim}0\%$ (Chronos)---is consistent with the normalization depth and type, not just RevIN presence.

\subsection{Top-\texorpdfstring{$k$}{k} Sparse Routing and Task Generalization}
\label{sec:topk}

Top-2 sparse routing retains $63\%$ of the dense mixture's MSE gain on ETTh1 while activating only $2$ of $5$ experts per sample, bounding active expert FLOPs to $40\%$ of the dense setting (full ablation in Appendix~\ref{app:topk}). On A10G (batch 128), RR-MoA Top-2 adds $4.3$\,ms ($+10\%$) latency and $2.6$\,MB peak memory over a single adapter---negligible compared to the $44.3$\,ms backbone forward pass (Appendix~\ref{app:benchmark}).

\textbf{Multi-horizon and task generalization.} RR-MoA wins \textbf{24/24} configurations across horizons $H \in \{96, 192, 336, 720\}$ on ETTh1 and ETTm1 (3 seeds each), with $-38\%$ to $-51\%$ MSE improvement at every horizon. The routing advantage extends to all 6 datasets across all 4 horizons (full grid in Appendix~\ref{app:horizon}), confirming that RR-MoA's benefit is not horizon-specific. RR-MoA also generalizes to imputation (20\% masked reconstruction, 3 seeds, mean$\pm$std): $-44\%$ to $-67\%$ MSE on 3/3 datasets (Appendix~\ref{app:imputation}).


\section{Conclusion}

We identified \emph{normalization-induced co-adaptation} as the cause of MoE routing collapse on TSFMs, proved via three converging controls and formalized via Proposition~\ref{thm:mi_decomp}, whose quantitative prediction is validated at $\rho{=}{-}0.96$ ($p{<}0.001$) across seven datasets. \textbf{RR-MoA} breaks the collapse by routing on the raw pre-normalization signal with a frozen backbone (\textbf{54/54 wins}, $p{<}0.001$, four backbones, four horizons, two tasks). The \emph{Frozen Paradox}---frozen RR-MoA outperforms full backbone fine-tuning by $16$--$77\%$ across 6 datasets---reverses the standard PEFT heuristic. A cross-backbone control on Chronos (no RevIN) confirms that RR-MoA helps when and only when normalization strips routing-relevant statistics, validating the theory's predictive scope. More broadly, we verify that the collapse generalizes beyond RevIN: replacing it with BatchNorm1d or GroupNorm produces the same entropy collapse (Appendix~\ref{app:norm_generalization}), establishing the normalization-routing tension as a property of \emph{any} instance-level normalizer. This predicts analogous failures wherever instance normalization precedes routing---including vision MoE with InstanceNorm and diffusion models with GroupNorm-gated experts.

\textbf{Limitations.} (1)~Absolute MSE gap to supervised baselines on MOMENT-small (narrows on Moirai). (2)~Native MoE backbones (Moirai-MoE, Time-MoE) not evaluated. (3)~Proposition~\ref{prop:frozen} is proved for a linear model; the nonlinear case remains open. (4)~Cross-modal generalization is empirically validated for three normalizer types on time series but not yet tested in other modalities.
