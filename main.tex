...
\begin{abstract}
Adapting pretrained time series foundation models (TSFMs) to downstream tasks currently relies on static, hand-designed adapter heads. While dynamically routing across diverse adapter topologies via mixture-of-experts (MoE) offers a path to task-specific specialization, we identify a fatal structural bottleneck: standard MoE routing collapses to a single expert (1/18 wins) because heavy backbone normalization destroys the hidden-state heterogeneity required for per-sample routing. We propose an end-to-end framework addressing both \textit{what} adapters to use and \textit{how} to route among them. First, we formalize \textbf{Adapter Architecture Search (AAS)} over an unbounded space of PyTorch \texttt{nn.Module} programs. AAS discovers novel cross-domain architectures---depthwise convolutions, BatchNorm gating, learned feature attention---that Pareto-dominate hand-designed baselines (63\% achieve both lower MSE and fewer parameters). Second, we propose \textbf{Raw-Routed Mixture of Adapters (RR-MoA)}, which bypasses the frozen backbone entirely for routing: a lightweight gating network reads the raw, unnormalized input signal to select among AAS-discovered experts that process the backbone's deep embeddings. RR-MoA achieves \textbf{17/18 wins on forecasting}---using the exact same experts that fail under standard routing---and \textbf{3/3 wins on imputation} with task-specific experts, yielding an overall \textbf{20/21 success rate with 56\% average MSE improvement}. We further show that zero-cost gradient proxies predict trained adapter MSE at initialization ($\rho{=}0.905$), amortizing the search cost to milliseconds.
\end{abstract}
...
\item \textbf{The routing collapse diagnosis.} We identify a fundamental incompatibility between standard MoE routing and pretrained TSFMs: backbone LayerNorm destroys hidden-state heterogeneity, collapsing the router to a single expert (1/18 wins). DARTS-style continuous NAS also fails due to a separate discretization gap (\S5).

\item \textbf{Raw-Routed Mixture of Adapters (RR-MoA).} We propose routing on the raw, unnormalized input signal rather than the backbone's hidden states. RR-MoA achieves \textbf{20/21 wins} across forecasting and imputation with an average \textbf{56\% MSE improvement}---using the same AAS-discovered experts that fail under standard routing (\S\ref{sec:rrmoa}, \S4.5).
...
\textbf{Code space $\gg$ discrete space.} All code-space methods outperform discrete evolutionary search (Evo-3epoch, 2,160 configurations) on 6/6 datasets (Wilcoxon signed-rank $p{=}0.031$, Cohen's $d{=}0.82$). The search space representation is the dominant factor.
...
\textbf{Search budget sufficiency.} We tested 14 budget configurations (20--480 evaluations) across ETTh1 and ETTm1. All beat the conv baseline, even at $B{=}20$ (3 minutes). We estimate $f \approx 0.03{-}0.05$, implying $P(\text{success}) = 1-(1-f)^B \geq 0.95$ for $B \geq 60$ (Appendix~\ref{app:budget}).
...
\textbf{AdaMix} (hidden-state routing) achieves 1/18 wins despite employing standard MoE regularization (auxiliary load-balancing loss, $\lambda{=}0.01$). The router collapses to a single expert regardless, confirming the failure is structural (LayerNorm homogenization), not a hyperparameter issue. Remarkably, routing collapse persists even when the top half of the backbone (4 of 8 blocks) is actively fine-tuned, proving that LayerNorm homogenization is an inherent bottleneck of the transformer forward pass. To ensure the performance delta stems from the routing signal rather than model capacity, AdaMix's gating network has identical parameter budget (${\sim}25$K) to the RR-MoA router. \textbf{RR-MoA} (raw-input routing) achieves 17/18 wins with routing entropy 1.43--1.55 / 1.61 max, confirming diverse, sample-dependent routing. Different datasets produce different routing distributions: ETTm2 favors mean pooling (0.35), while ETTh2 favors last-token (0.31).

\textbf{Task generalization: Imputation.} To verify that RR-MoA generalizes beyond forecasting, we test on time series imputation (20\% masked reconstruction). Because imputation requires upsampling from patches to timesteps rather than temporal pooling, we executed AAS in $\mathcal{G}_\text{code}$ on the imputation objective. AAS discovered dense, per-patch reconstruction motifs (linear upsample, dense MLP, transposed conv, attention upsample, residual conv), which we used to populate a task-specific RR-MoA expert pool. RR-MoA achieves --43\% (ETTh1), --52\% (ETTm1), and --69\% (Weather) MSE improvement over the best single head. Crucially, the winning architecture \textit{differs by task}: forecasting favors temporal pooling, imputation favors per-patch dense reconstruction---confirming that adapter architecture is both dataset- and task-dependent (Table~\ref{tab:imputation}).
...
To further reduce the cost of populating the expert pool, we evaluate zero-cost architecture scoring at initialization. The gradient L2-norm $\|\nabla_\theta \mathcal{L}\|_2$ computed on a single batch (Epoch~0, no training) correlates strongly with the 15-epoch test MSE on 3/6 datasets: Spearman $\rho{=}0.905$ ($p{=}0.002$) on Electricity, $\rho{=}0.762$ ($p{=}0.028$) on Weather, and $\rho{=}0.738$ ($p{=}0.037$) on ETTm2. This enables scoring and ranking adapter architectures in ${\sim}300$ms without training, reducing search cost by $6{,}000\times$ on compatible datasets. The proxy fails on ETTh1 and ETTm1. We hypothesize this is tied to dataset stationarity: Electricity and Weather exhibit stable periodic patterns where Epoch~0 gradient geometry already reflects the converged loss landscape, whereas ETTh1's distribution shifts render initialization gradients less predictive. Full scatter plots in Appendix~\ref{app:tzcp}.

\section{Discussion}

\textbf{The time-series discretization gap.} Continuous NAS fails on frozen TSFMs for two distinct reasons. First, in preliminary experiments applying continuous relaxation to our adapter space, DARTS~\citep{liu2019darts} suffers a severe discretization gap (1/6 wins): the soft mixture learns co-adapted operation weights that break when snapped to hard selections. Second, MoE routing collapses (1/18 wins, 6\%): TSFM LayerNorm homogenizes hidden states, destroying the per-sample spectral heterogeneity needed for routing. This second failure is \textit{specific to frozen TSFMs}---vision transformers retain token-level heterogeneity. RR-MoA resolves the routing collapse by bypassing the backbone (17/18 wins), proving the diagnosis is correct and the fix is sound.

\textbf{Search space and efficiency.} Two discrete search operators (random sampling and evolutionary selection) perform similarly in $\mathcal{G}_\text{tmpl}$ because the quality fraction $f \approx 0.03{-}0.05$ ensures random sampling suffices at $B{\geq}60$. The LLM, searching the unbounded $\mathcal{G}_\text{code}$, discovers patterns \textit{outside} $\mathcal{G}_\text{tmpl}$ that are 19.6\% better ($p{=}0.003$)---these serve as the diverse expert pool in RR-MoA. Of 30 evolved adapters, 63\% Pareto-dominate the conv baseline (lower MSE \textit{and} fewer parameters). AAS automatically imports cross-domain patterns---depthwise convolutions~\citep{howard2017mobilenets}, BatchNorm~\citep{he2016resnet, ioffe2015batchnorm}, channel attention~\citep{hu2018senet}---mirroring cross-modal transfer findings~\citep{shen2025visionts}.

\textbf{Limitations.} (1)~Frozen-backbone MSEs (${\sim}1.0$ on ETTh1) are higher than full fine-tuning (${\sim}0.37$ for PatchTST~\citep{nie2023patchtst}); our contribution is relative improvement \textit{within} this paradigm. (2)~Architectures transfer to only 35\% of other datasets (Appendix~\ref{app:transfer}). (3)~H=336/720 improvements are marginal (--0.1\% to --0.6\%).
