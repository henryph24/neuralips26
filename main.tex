...
\item \textbf{Raw-Routed Mixture of Adapters (RR-MoA).} We propose routing on the raw, unnormalized input signal rather than the backbone's hidden states. RR-MoA achieves 17/18 wins on forecasting using the exact same experts that fail under standard routing, plus 3/3 wins on imputation with task-specific experts---\textbf{20/21 overall with 56\% average MSE improvement} (\S\ref{sec:rrmoa}, \S4.5).
...
\node[block=blue!12, minimum width=4.2cm, minimum height=1.1cm] (backbone) at (-2.8, -1.8) {
    \textbf{Pretrained TSFM Backbone} $f_\theta$\\[-2pt]
...
\begin{equation}
g^* = \arg\min_{g \in \mathcal{G}} \; \mathcal{L}\!\left(g_{\hat{\phi}(g)}, \; \mathcal{D}_\text{val}\right), \quad \text{where} \quad \{\hat{\phi}, \hat{\theta}_\text{act}\} = \arg\min_{\phi, \theta_\text{act}} \; \mathcal{L}\!\left(g_\phi \circ f_{\theta}, \; \mathcal{D}_\text{train}\right)
\label{eq:aas}
\end{equation}
...
\end{equation}
The full Cartesian product yields ${\sim}2{,}160$ configurations including backbone-level choices (LoRA rank, layer placement, unfreeze strategy), of which ${\sim}144$ define distinct adapter \textit{computational graphs}. Each graph $c \mapsto g_c$ is a fixed pooling + projection pipeline.
\end{definition}
...
\textbf{The fix: route on raw input.} We propose \textbf{RR-MoA}, which splits the information stream: the pretrained TSFM processes the deep semantic representation $\mathbf{H}$, while a lightweight gating network reads the \textit{raw, unnormalized} input $\mathbf{X}$ to compute routing probabilities:
\begin{equation}
\mathbf{Y} = \sum_{k=1}^K \underbrace{\text{Softmax}\!\left(G_\psi(\mathbf{X}_\text{raw})\right)_k}_{\text{raw-signal routing}} \cdot \underbrace{\text{Expert}_k(\mathbf{H}_\text{FM})}_{\text{deep embeddings}}
\label{eq:rrmoa}
\end{equation}
where $G_\psi$ is a small Conv1d + pooling + linear network (${\sim}25$K params) operating on the raw time series, and each Expert$_k$ is an AAS-discovered adapter head. The raw input preserves the spectral and statistical diversity (autocorrelation, trend, kurtosis) that the backbone normalizes away, enabling per-sample routing that reflects the physical characteristics of each time series window.

\begin{algorithm}[h]
\caption{RR-MoA: Raw-Routed Mixture of Adapters}
\label{alg:rrmoa}
\begin{algorithmic}[1]
\REQUIRE Pretrained TSFM $f_\theta$, raw input $\mathbf{X} \in \mathbb{R}^{B \times L}$, $K$ expert adapters
\STATE $\mathbf{H} \leftarrow f_\theta(\mathbf{X})$ \hfill \COMMENT{Deep embeddings (normalized)}
\STATE $\mathbf{w} \leftarrow \text{Softmax}(G_\psi(\mathbf{X}))$ \hfill \COMMENT{Raw-signal routing weights}
\FOR{$k = 1$ to $K$}
...
\midrule
Best single expert & 1.10 & 0.85 & 2.74 & 2.95 & 0.47 & 0.47 \\
...
\midrule
$\Delta$ (RR-MoA vs best expert) & --50\% & --49\% & --74\% & --87\% & --34\% & --47\% \\
\bottomrule
\end{tabular}
\end{table}
