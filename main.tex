...
\end{itemize}
Critically, all of these share the same underlying computational graph: temporal pooling followed by dense projection. No existing TSFM adapter uses convolutional feature extraction, learned channel attention, batch normalization, or depthwise separable operations---architectural patterns that are standard in vision~\cite{howard2017mobilenets, he2016resnet, hu2018senet} but absent from the time series adapter vocabulary. The only searchable parameters are scalar hyperparameters (LoRA rank, bottleneck dimension, number of MLP layers, pooling type), yielding a discrete space of ${\sim}2{,}000$ configurations with two limitations. First, novel structural ideas cannot be discovered if they are absent from the menu. Second, the space is small enough that random search achieves near-optimal coverage, leaving no room for intelligent exploration.
