...
\textbf{Why frozen at all?} For a single dataset at a single horizon, full-fine-tuned specialists such as PatchTST~\citep{nie2023patchtst} and even a linear-from-scratch DLinear~\citep{zeng2023dlinear} obtain lower absolute MSE than any frozen-backbone adapter. Our contribution is \emph{not} to replace these in the single-dataset setting. The frozen-backbone paradigm we target is motivated by three deployment regimes in which full fine-tuning is infeasible and in which re-training a DLinear per use-case is operationally expensive: (i)~\textbf{multi-tenant cloud serving}, where a single TSFM sits in GPU VRAM and serves thousands of tenants via lightweight, hot-swappable per-tenant adapters in host RAM---retraining a separate DLinear per tenant does not amortize the backbone cost and forfeits any zero-shot transfer the TSFM provides; (ii)~\textbf{on-device and edge deployment}, where one backbone is shipped once with the device image and per-site adapters are updated over low-bandwidth links---DLinear per site requires collecting each site's training data at a central location, which is often impossible for privacy or latency reasons; and (iii)~\textbf{cross-task reuse}, where the same frozen backbone drives forecasting, imputation, and (in principle) classification via different expert pools---we demonstrate the first two of these explicitly in \S\ref{sec:rrmoa} and \S4.5. Within this paradigm, the question is \emph{not} whether the frozen TSFM beats a specialist trained from scratch on the same dataset (it often will not), but which adapter / routing strategy minimises the downstream error for the fixed backbone that practitioners are committed to. That is the question we answer.
...
\caption{\textbf{RR-MoA architecture.} The raw input splits into two streams. \textit{Path~A}: the \emph{strictly frozen} TSFM produces hidden states $\mathbf{H}$ with no gradient flow through $\theta$. \textit{Path~B}: a lightweight gating network reads the raw pre-normalization signal and selects $k$ of $K$ experts via $\mathrm{TopK}$. Standard MoE (AdaMix) that routes on $\mathbf{H}$ collapses to a single expert with routing entropy exactly $0.000\pm0.000$ across 3 seeds whenever any backbone layer is unfrozen (Table~\ref{tab:adamix}); RR-MoA achieves \textbf{27/27 wins} across 3 datasets $\times$ 3 freeze levels $\times$ 3 seeds (Table~\ref{tab:rrmoa}).}
\label{fig:framework}
\end{figure}
...
\node[block=orange!15] (a2) at (0, 0.4) {DepthConv1d\\groups$=d$, $k{=}3$};
