...
\item \textbf{Empirical finding.} Even the simplest AAS instantiation---random template search with elitism, 30 minutes on a single GPU---discovers task-specific adapters that beat hand-designed baselines on \textbf{all 19 experiments} across 5 datasets, 3 seeds, and 4 TSFM backbones, with an average improvement of 8--11\%. The simplicity of the method strengthens the finding: the design space is so rich that no sophistication is needed to outperform the status quo (\S\ref{sec:main_results}).
...
Tables~\ref{tab:main} and~\ref{tab:backbone} show results on the held-out test set. AAS discovers task-specific adapter architectures that beat the best fixed adapter on \textbf{all 19 experiments}.

\begin{table}[h]
\centering
\caption{Standard LTSF benchmark on MOMENT-small (test MSE). AAS wins all 14 experiments across 5 datasets, multiple seeds, and horizons (H=96--720). Best fixed adapter is \textit{conv} (Conv1d + Linear, 209--290K params). All comparisons use identical training setup; only the adapter architecture differs.}
\label{tab:main}
\small
...
\midrule
\multicolumn{6}{l}{\textit{Other datasets (seed 42, H=96)}} \\
ETTh2 & seeds 42/43/44 & \textbf{2.74}/\textbf{2.47}/\textbf{3.07} & 3.10/2.97/3.13 & --11.9\%/--16.9\%/--1.8\% & 79K \\
ETTm2 & seed 42 & \textbf{2.8284} & 3.1059 & --8.9\% & 104K \\
Weather & seed 42 & \textbf{0.4933} & 0.6085 & --18.9\% & 158K \\
\midrule
\multicolumn{2}{l}{\textit{Average (14 experiments)}} & & & \textbf{--8.4\%} & \\
\bottomrule
\end{tabular}
\end{table}
...
Two patterns emerge. First, improvement is consistent: AAS wins on every dataset, seed, and horizon, with the largest gains on ETTm1 seed 43 (--21.0\%) and Weather (--18.9\%), and the smallest on ETTh1 H=336 (--0.1\%). The H=336 margins are notably tighter than H=96, suggesting that longer horizons leave less room for architectural advantage. Second, AAS discovers more \textit{efficient} architectures: mean 120K params vs.\ 209K for the best fixed baseline. Multi-seed results on ETTh1, ETTm1, and ETTh2 (3 seeds each) confirm robustness.
