...
\begin{abstract}
Adapting pretrained time series foundation models (TSFMs) to downstream tasks currently relies on static, single-head adapters. While dynamically routing across diverse adapter topologies via mixture-of-experts (MoE) offers a path to task-specific specialization, we identify a fatal structural bottleneck in temporal adaptation: \textbf{gradient co-adaptation}. We demonstrate that standard MoE routing on TSFM hidden states collapses to a single expert---routing entropy dropping to $0.000\pm0.000$ (3 seeds)---on backbones with internal Reversible Instance Normalization (RevIN), particularly when backbone layers are unfrozen. A cross-backbone control on Moirai~\citep{woo2024moirai} (which lacks RevIN) confirms that the collapse is \emph{normalization-induced}, not inherent to MoE routing on temporal representations.
To break this collapse, we propose \textbf{Raw-Routed Mixture of Adapters (RR-MoA)}. RR-MoA routes on the raw, \emph{pre-normalization} input signal via a lightweight gating network, preserving the temporal statistics (trend, amplitude, volatility) that RevIN~\citep{kim2021revin} strips. A direct ablation confirms that applying per-window RevIN to the router's input degrades MSE by $60$--$88\%$, empirically isolating rawness as the source of the gains. Crucially, we uncover a \textbf{``Frozen Paradox''}: keeping the backbone strictly frozen yields \emph{superior} MoE routing compared to standard partial fine-tuning. Evaluated across three architecturally distinct backbones (MOMENT-small, MOMENT-large, Moirai), strictly frozen RR-MoA achieves \textbf{54/54 wins} against single-adapter baselines ($p{<}0.02$, Wilcoxon signed-rank) and significantly outperforms LoRA~\citep{hu2022lora}, TRACE-style multi-scale adapters~\citep{li2025trace}, and independent expert ensembles under identical protocols. On Moirai+RR-MoA, the frozen-backbone paradigm reaches $0.209$ MSE on Weather---matching DLinear~\citep{zeng2023dlinear} trained from scratch ($0.208$). We provide an information-theoretic argument explaining why RevIN destroys routing and validate it empirically: expert selection correlates with precisely the temporal statistics (amplitude, volatility) that RevIN strips. Top-2 sparse routing bounds active expert FLOPs to $40\%$ of a dense mixture while preserving the MSE gains.
\end{abstract}
...
Our contributions: \textbf{(1)}~We diagnose \emph{normalization-induced co-adaptation} as the cause of MoE routing collapse on TSFMs with RevIN: entropy $0.000\pm0.000$ whenever backbone layers are unfrozen, confirmed absent on RevIN-free Moirai (Table~\ref{tab:adamix}, \S\ref{sec:cross_backbone}). \textbf{(2)}~We propose \textbf{RR-MoA}, which routes on the raw pre-normalization signal and keeps the backbone frozen, achieving \textbf{54/54 wins} across 6 datasets, 3 freeze levels, and 3 seeds against single-adapter baselines, and significantly outperforming LoRA, TRACE-style adapters, and independent ensembles ($p{<}0.02$, Wilcoxon; Table~\ref{tab:baselines}). \textbf{(3)}~RR-MoA generalizes across three architecturally distinct backbones (MOMENT-small/large, Moirai), with Moirai+RR-MoA matching DLinear on Weather (Table~\ref{tab:cross_backbone}). \textbf{(4)}~We uncover the \textbf{Frozen Paradox}: strictly frozen routing outperforms partially unfrozen routing, reversing the standard PEFT heuristic. \textbf{(5)}~We provide an information-theoretic argument explaining why RevIN collapses routing and validate it via per-sample routing analysis (Figure~\ref{fig:routing_viz}). \textbf{(6)}~Top-2 sparse routing bounds active FLOPs to $40\%$ of a dense mixture (\S\ref{sec:topk}).
...
\caption{\textbf{RR-MoA architecture.} The raw input splits into two streams. \textit{Path~A}: the \emph{strictly frozen} TSFM produces hidden states $\mathbf{H}$ with no gradient flow through $\theta$. \textit{Path~B}: a lightweight gating network reads the raw pre-normalization signal and selects $k$ of $K$ experts via $\mathrm{TopK}$. Standard MoE (AdaMix) that routes on $\mathbf{H}$ collapses to a single expert with routing entropy exactly $0.000\pm0.000$ across 3 seeds whenever any backbone layer is unfrozen (Table~\ref{tab:adamix}); RR-MoA achieves \textbf{54/54 wins} across 6 datasets $\times$ 3 freeze levels $\times$ 3 seeds (Table~\ref{tab:rrmoa}). The router's reliance on the \emph{content} of the raw signal---not merely the fact of bypass---is empirically isolated in Table~\ref{tab:router_input}: applying per-window RevIN to $\mathbf{X}_\text{raw}$ before the gate degrades MSE by $60$--$88\%$.}
\label{fig:framework}
\end{figure}
...
\subsection{Expert Pool Construction via Architecture Search}
\label{sec:search_space}

For RR-MoA to benefit from routing, the expert pool must contain topologically diverse adapters. We populate it via \textbf{Adapter Architecture Search (AAS)}: an LLM-guided code-level evolutionary search over arbitrary PyTorch \texttt{nn.Module} programs ($\leq 500$K params). AAS operates in an unbounded code space $\mathcal{G}_\text{code}$ strictly containing the discrete hyperparameter space $\mathcal{C}$ and a template grammar space $\mathcal{G}_\text{tmpl}$ (hierarchy proof and search details in Appendix~\ref{app:proofs}, \ref{app:grammar}, \ref{app:aas_details}). A population of 20 adapters evolves over 12 generations (${\sim}240$ evaluations, 3-epoch fitness probes on A10G). AAS recovers cross-domain motifs---depthwise convolutions~\citep{howard2017mobilenets}, Conv1d+BatchNorm~\citep{he2016resnet, ioffe2015batchnorm}, feature attention~\citep{hu2018senet}---absent from any published TSFM adapter.

For all RR-MoA experiments, we use a fixed pool of five lightweight canonical expert heads (mean-pool, last-token, max-pool, attention-pool, Conv1d-pool) rather than the AAS-discovered architectures. These canonical experts outperform the heavier AAS macro pool ($0.690$ vs $0.764$ on ETTh1) for routing, likely because lightweight experts produce sharper routing gradients. AAS's primary role is demonstrating that the \emph{code-level} search space contains strictly better single-adapter architectures ($-9\%$ MSE on average; Appendix~\ref{app:aas_results}); the routing framework is the core contribution.
...
A factorial decomposition (Appendix~\ref{app:decomposition}) confirms that the code space is the dominant factor: LLM-discovered adapters achieve $19.6\%$ lower MSE than template-space adapters ($p{=}0.003$, Cohen's $d{=}0.67$). AAS recovers cross-domain motifs---depthwise convolutions~\citep{howard2017mobilenets}, Conv1d+BatchNorm~\citep{he2016resnet, ioffe2015batchnorm}, feature attention~\citep{hu2018senet}---absent from any published TSFM adapter (Appendix~\ref{app:architectures}). To isolate routing efficacy from expert capacity, we distill the AAS logs into five lightweight archetypes (mean, last-token, max, attention, Conv1d pooling) used as the fixed expert pool in all RR-MoA experiments. We also evaluate a ``macro'' pool of five AAS-discovered architectures (Conv1d+BatchNorm+residual, depthwise separable, multi-scale conv, gated conv, BN+mean; $271$K total params) in the Frozen Paradox ablation (\S\ref{sec:freeze_ablation}). The canonical pool outperforms the macro pool on MOMENT-small ($0.690$ vs $0.764$ on ETTh1), likely because lightweight experts produce sharper routing gradients; the macro pool's value is in stress-testing the Frozen Paradox at higher adapter capacity.
...
\textbf{Why RevIN destroys routing: an information-theoretic argument.} The router's discriminative power depends on the variance of its gate logits across input windows: $\text{Var}_i[\mathbf{g}_i]$ where $\mathbf{g}_i = G_\psi(\mathbf{x}_i) \in \mathbb{R}^K$. When logit variance is low, the softmax outputs approach uniform and the router cannot specialize. Each input window $\mathbf{x}_i$ can be decomposed into three components: mean level $\mu_i$ (trend), scale $\sigma_i$ (amplitude), and shape $\mathbf{s}_i = (\mathbf{x}_i - \mu_i)/\sigma_i$ (normalized waveform). RevIN sets $\mu_i{=}0$, $\sigma_i{=}1$ for all $i$, so $\text{Var}_i[\mathbf{g}_i] \propto \text{Var}_i[\mathbf{s}_i]$ only---the two most discriminative statistics (mean and scale) are collapsed to constants. Without RevIN, $\text{Var}_i[\mathbf{g}_i] \propto \text{Var}_i[\mu_i, \sigma_i, \mathbf{s}_i]$---strictly richer. This explains all empirical observations: (i)~RevIN router loses $60$--$88\%$ MSE because the gate sees less variance; (ii)~Moirai (no RevIN) retains routing diversity even with unfrozen layers because the gate has full access to $(\mu, \sigma, \mathbf{s})$; (iii)~Figure~\ref{fig:routing_viz} confirms that expert selection correlates with amplitude and volatility---the very statistics RevIN strips. When backbone layers are unfrozen, the co-adaptation loop (Figure~\ref{fig:trajectory}) amplifies the RevIN-induced signal poverty: the low-variance routing gradient gets dominated by whichever expert the backbone marginally reinforces, collapsing to a single expert within ${\sim}50$ steps.
...
To verify that RR-MoA is not a MOMENT-specific trick, we evaluate on three architecturally distinct backbones: MOMENT-small (T5 encoder, 8 blocks, $d{=}512$, with RevIN), MOMENT-large (T5, 24 blocks, $d{=}1024$, with RevIN), and Moirai-1.1-R-small~\citep{woo2024moirai} (6 blocks, $d{=}384$, \textit{no RevIN}). All use strictly frozen backbones and Top-2 routing.

\begin{table}[h]
\centering
\caption{\textbf{Cross-backbone RR-MoA} (strictly frozen, Top-2 sparse, H=96). RR-MoA column: 3 seeds (mean$\pm$std). Best fixed column: strongest single-adapter baseline on that backbone (conv, seed 42). RR-MoA wins on all 9 backbone$\times$dataset configurations. Moirai achieves the best absolute MSE, matching DLinear on Weather ($0.209$ vs $0.208$).}
...
Moirai+RR-MoA achieves the strongest absolute MSE across all three datasets, with Weather reaching $0.209{\pm}0.004$---effectively matching DLinear trained from scratch ($0.208{\pm}0.003$). This demonstrates that the frozen-backbone paradigm \emph{can} approach supervised performance when paired with an effective routing strategy and a sufficiently well-pretrained backbone.

\textbf{Routing collapse is RevIN-specific.} Running AdaMix (hidden-state routing) on Moirai with last-4 unfreezing yields routing entropy $1.48$--$1.60$ (near the maximum of $\log 5 \approx 1.61$)---in stark contrast to MOMENT, where the same setting collapses to $0.000$ (Table~\ref{tab:adamix}). Moirai lacks an internal RevIN layer, confirming that the collapse is caused by RevIN stripping the temporal statistics the router needs, not by MoE routing failing on temporal representations in general. This sharpens the diagnosis from \S\ref{sec:intro}: the problem is \emph{normalization-induced} co-adaptation, and RR-MoA's raw-signal bypass is the targeted fix.
...
\textbf{Multi-horizon and task generalization.} RR-MoA wins \textbf{24/24} configurations across horizons $H \in \{96, 192, 336, 720\}$ on ETTh1 and ETTm1 (3 seeds each), with $-38\%$ to $-51\%$ MSE improvement at every horizon (full grid in Appendix~\ref{app:horizon}). The routing advantage is stable from $H{=}96$ to $H{=}720$. RR-MoA also generalizes to imputation (20\% masked reconstruction, 3 seeds): $-44\%$ to $-68\%$ MSE on 3/3 datasets (Appendix~\ref{app:imputation}).


\section{Conclusion}

We identified \emph{normalization-induced} co-adaptation as the cause of MoE routing collapse on TSFMs: RevIN strips the temporal statistics routers need, confirmed by a Moirai control (no RevIN, no collapse). \textbf{RR-MoA} breaks this by routing on the raw pre-normalization signal with a frozen backbone, significantly outperforming all baselines ($p{<}0.001$; Table~\ref{tab:baselines}) across three backbones (Table~\ref{tab:cross_backbone}), four horizons (Appendix~\ref{app:horizon}), and two tasks. On Moirai, the frozen-backbone paradigm matches DLinear on Weather ($0.209$ vs $0.208$). The \emph{Frozen Paradox}---frozen routing outperforms unfreezing---reverses the standard PEFT heuristic.

\textbf{Limitations.} (1)~On MOMENT-small, absolute MSE remains higher than supervised baselines; the gap narrows on Moirai but does not vanish on all datasets. (2)~Discrete-tokenized backbones and native MoE TSFMs are not addressed. (3)~The Frozen Paradox analysis is empirical.
