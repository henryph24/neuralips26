...
\textbf{The Frozen Paradox.} Across all 6 datasets (Appendix~\ref{app:extended_rrmoa}), strictly frozen is the best configuration on 4 and within $13\%$ on the remaining 2, \emph{reversing} the standard PEFT heuristic. The strongest test: frozen RR-MoA beats \emph{full backbone fine-tuning} (all 8 blocks unfrozen, best of 5 heads $\times$ 2 learning rates) by $16$--$77\%$ on every dataset (Table~\ref{tab:baselines}). The mechanism is Table~\ref{tab:adamix}: unfreezing destroys routing diversity via the gradient co-adaptation feedback loop (Proposition~\ref{prop:frozen}).
...
\textbf{Cross-backbone generalization.}\label{sec:cross_backbone} We evaluate RR-MoA on four architecturally distinct backbones spanning encoder-only and encoder-decoder architectures:

\textit{Backbones with RevIN} (MOMENT-small/large): RR-MoA wins \textbf{all 18 configurations} (6 datasets $\times$ 3 seeds on each backbone). Moirai+RR-MoA extends to all 6 datasets, winning \textbf{18/18} with improvements of $3$--$31\%$ (full table in Appendix~\ref{app:cross_backbone}). Moirai+RR-MoA on Weather reaches $0.209{\pm}0.004$---matching DLinear from scratch ($0.208$).

\textit{Backbone without RevIN} (Chronos-T5-small): RR-MoA wins only \textbf{1/9} configurations. This is \emph{predicted by the theory}: Chronos has no RevIN, so hidden states retain per-window statistics and the conv baseline already achieves strong MSE ($0.659$ on ETTh1, $0.220$ on Weather). The routing advantage is specific to the normalization problem, not a general property of MoE. AdaMix on Chronos with last-4 unfreezing retains entropy $1.48$--$1.60$ (no collapse), independently confirming the failure is RevIN-specific.

Across all four backbones, the pattern is consistent: RR-MoA helps when and only when the backbone strips routing-relevant statistics via normalization.
