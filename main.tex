...
\textbf{Cross-backbone generalization.}\label{sec:cross_backbone} We evaluate RR-MoA on four backbones, revealing a \emph{normalization spectrum}: MOMENT-small/large (RevIN): \textbf{all 18 configs} win per backbone ($27$--$81\%$ improvement); Moirai (no RevIN, LayerNorm only): \textbf{18/18} wins ($3$--$31\%$; Moirai+RR-MoA matches DLinear on Weather: $0.209$ vs $0.208$); Chronos (no RevIN, encoder-decoder): only \textbf{1/9} wins, with AdaMix retaining entropy $1.48$--$1.60$ (no collapse)---independently confirming collapse is normalization-specific (full tables in Appendix~\ref{app:cross_backbone}). The improvement gradient $81\%{>}31\%{>}{\sim}0\%$ tracks normalization aggressiveness, not backbone size.
...
We identified \emph{normalization-induced co-adaptation} as the cause of MoE routing collapse on TSFMs, proved via three converging controls and formalized via Proposition~\ref{thm:mi_decomp} ($\rho{=}{-}0.96$, $p{<}0.001$ across seven datasets). \textbf{RR-MoA} breaks the collapse by routing on raw pre-normalization signal with a frozen backbone (\textbf{54/54 wins}, $p{<}0.001$, four backbones, four horizons, two tasks). The \emph{Frozen Paradox}---frozen RR-MoA outperforms full fine-tuning by $16$--$77\%$---reverses the standard PEFT heuristic. The collapse generalizes beyond RevIN to BatchNorm1d and GroupNorm (Appendix~\ref{app:norm_generalization}), predicting analogous failures wherever instance normalization precedes routing in vision MoE and diffusion models.

\textbf{Limitations.} (1)~Absolute MSE gap to supervised baselines on MOMENT-small (narrows with horizon and backbone quality; Appendix~\ref{app:diagnostic} confirms genuine information loss). (2)~Native MoE backbones (Moirai-MoE, Time-MoE) not evaluated. (3)~Proposition~\ref{thm:mi_decomp} proved for a linear model; nonlinear case open. (4)~Normalization generalization validated on time series only. (5)~Expert pool hand-selected; cross-channel routing unexplored.
...
All three normalizers cause entropy collapse to ${\leq}0.004$, while removing normalization entirely keeps entropy stable at $0.817$. The mechanism is the same in each case: the normalizer strips per-window statistics before the encoder, the router receives homogenized hidden states, and gradient co-adaptation amplifies the initial routing bias into full collapse. This confirms that the normalization-routing tension is a \emph{general} property of instance-level normalization, not a RevIN-specific artifact, and predicts analogous failures in other modalities wherever instance normalization precedes routing decisions.
