...
% ====== INPUT ======
\node[block=gray!8, minimum width=\columnwidth-1cm, minimum height=1.1cm] (input) at (0, 0) {
    \textbf{Raw Time Series Input} \quad $\mathbf{X}_\text{raw} \in \mathbb{R}^{B \times L}$
};

% ====== TWO PATHS (side by side) ======
% Left path: Backbone
\node[block=blue!12, minimum width=4.8cm, minimum height=1.1cm] (backbone) at (-3.1, -1.8) {
    \textbf{Frozen TSFM Backbone} $f_\theta$\\[-2pt]
    {\scriptsize LayerNorm $\times K$ layers}
};
\node[block=blue!6, minimum width=4.8cm] (hidden) at (-3.1, -3.2) {
    Hidden States $\mathbf{H} \in \mathbb{R}^{B \times T \times d}$\\[-2pt]
    {\scriptsize \textcolor{red!70}{homogenized by LayerNorm}}
};

% Right path: Router
\node[block=orange!12, minimum width=4.8cm, minimum height=1.1cm] (router) at (3.1, -1.8) {
    \textbf{Raw-Signal Router} $G_\phi(\mathbf{X}_\text{raw})$\\[-2pt]
    {\scriptsize Conv1d $\to$ AdaptivePool $\to$ Linear}
};
\node[block=orange!6, minimum width=4.8cm] (weights) at (3.1, -3.2) {
    Routing Weights $\mathbf{w} \in \Delta^K$\\[-2pt]
    {\scriptsize \textcolor{green!50!black}{preserves spectral diversity}}
};

% ====== EXPERT POOL ======
\node[block=purple!8, minimum width=\columnwidth-1cm, minimum height=1.2cm] (experts) at (0, -4.8) {
    \textbf{AAS-Discovered Expert Pool} ($K{=}5$ diverse adapters)\\[-1pt]
    {\small \fbox{MeanPool} \quad \fbox{LastToken} \quad \fbox{MaxPool} \quad \fbox{Attention} \quad \fbox{Conv1d}}
};

% ====== OUTPUT ======
\node[block=green!8, minimum width=\columnwidth-1cm, minimum height=0.9cm] (output) at (0, -6.3) {
...
% ====== LABELS on paths ======
\node[font=\footnotesize\bfseries, text=blue!60] at (-3.1, -0.85) {Path A: Deep Semantics};
\node[font=\footnotesize\bfseries, text=orange!60] at (3.1, -0.85) {Path B: Physical Routing};
...
\end{tikzpicture}
\caption{\textbf{Raw-Routed Mixture of Adapters (RR-MoA).} The raw input $\mathbf{X}$ splits into two information streams. \textit{Path~A}: the frozen TSFM produces deep hidden states $\mathbf{H}$, but LayerNorm homogenizes them across samples. \textit{Path~B}: a lightweight gating network reads the raw, unnormalized signal to compute per-sample routing weights $\mathbf{w}$. Standard MoE routing on $\mathbf{H}$ collapses the router to a single expert (1/18 wins, center \ding{55}). Raw routing preserves spectral diversity and achieves \textbf{17/18 wins with 56\% MSE improvement}.}
\label{fig:framework}
\end{figure}
...
\textbf{AdaMix} (hidden-state routing) achieves 1/18 wins despite employing standard MoE regularization (auxiliary load-balancing loss, $\lambda{=}0.01$). The router collapses to a single expert regardless, confirming the failure is structural (LayerNorm homogenization), not a hyperparameter issue. \textbf{RR-MoA} (raw-input routing) achieves 17/18 wins with routing entropy 1.43--1.55 / 1.61 max, confirming diverse, sample-dependent routing. Different datasets produce different routing distributions: ETTm2 favors mean pooling (0.35), while ETTh2 favors last-token (0.31).

\textbf{Task generalization: Imputation.} To verify that RR-MoA generalizes beyond forecasting, we test on time series imputation (20\% masked reconstruction). Because imputation requires upsampling from patches to timesteps rather than temporal pooling, we populate the RR-MoA pool with 5 imputation-specific archetypes (linear upsample, dense MLP, transposed conv, attention, residual conv). RR-MoA achieves --43\% (ETTh1), --52\% (ETTm1), and --69\% (Weather) MSE improvement over the best single imputation head. Crucially, the winning architecture \textit{differs by task}: forecasting favors temporal pooling (attention, last-token), while imputation favors per-patch dense reconstruction. This confirms that adapter architecture is both dataset- and task-dependent.
...
\section{Routing Entropy: AdaMix vs.\ RR-MoA}
\label{app:routing}
...
\begin{axis}[
    ...
    ylabel={Mean routing weight},
    ylabel style={font=\small},
    symbolic x coords={ETTh1, ETTm1, ETTh2, ETTm2, Weather, Electricity},
    xtick=data,
    x tick label style={font=\small},
    y tick label style={font=\small},
    legend style={at={(0.5,0.98)}, anchor=north, legend columns=5, font=\tiny},
...
% RR-MoA routing weights (seed 42) — 5 experts
\addplot[fill=blue!60] coordinates {(ETTh1,0.234) (ETTm1,0.237) (ETTh2,0.365) (ETTm2,0.346) (Weather,0.287) (Electricity,0.237)};
\addplot[fill=red!40] coordinates {(ETTh1,0.166) (ETTm1,0.178) (ETTh2,0.125) (ETTm2,0.164) (Weather,0.164) (Electricity,0.162)};
\addplot[fill=green!40] coordinates {(ETTh1,0.189) (ETTm1,0.179) (ETTh2,0.208) (ETTm2,0.258) (Weather,0.209) (Electricity,0.181)};
\addplot[fill=orange!40] coordinates {(ETTh1,0.227) (ETTm1,0.226) (ETTh2,0.217) (ETTm2,0.140) (Weather,0.210) (Electricity,0.253)};
\addplot[fill=purple!30] coordinates {(ETTh1,0.184) (ETTm1,0.179) (ETTh2,0.085) (ETTm2,0.092) (Weather,0.131) (Electricity,0.168)};
\legend{Mean, Last, Max, Attn, Conv}
\end{axis}
\end{tikzpicture}
\caption{\textbf{RR-MoA routing weight distribution} (seed 42). Each bar group shows the mean routing weight for each expert across test samples. Routing is diverse and dataset-dependent: ETTh2 favors MeanPool (0.37), ETTm2 favors MaxPool (0.26), Electricity favors Attention (0.25). Routing entropy ranges 1.43--1.55 / 1.61 max. \textbf{AdaMix} (not shown) collapses to a single expert with entropy $\approx 0$, assigning $>$0.99 weight to one expert regardless of input.}
\label{fig:routing}
\end{figure}
...
\begin{proposition}[Strict Hierarchy --- Full Proof]
$\mathcal{C} \subset \mathcal{G}_\emph{tmpl} \subset \mathcal{G}_\emph{code}$, and both inclusions are strict.
\end{proposition}
...
From our budget experiments (Figure~\ref{fig:budget}), we estimate $f \approx 0.03{-}0.05$, implying $B \geq 60$ suffices for ${\geq}95\%$ success. This explains: (i)~$B{=}20$ achieves ${\sim}64\%$ success; (ii)~$B{=}60$ achieves near-certain success; (iii)~evolutionary selection adds no value when $B \gg 1/f$.
...
\begin{table}[h]
\centering
\caption{Imputation task (20\% masked reconstruction). RR-MoA wins 3/3 with diverse routing. Forecasting prefers temporal pooling; imputation prefers per-patch reconstruction.}
\small
\begin{tabular}{@{}lccccc@{}}
\toprule
Dataset & Linear & Dense MLP & Conv & Attention & \textbf{RR-MoA} \\
\midrule
ETTh1 & 0.242 & 0.339 & 0.254 & — & \textbf{0.138} \\
ETTm1 & — & 0.218 & — & — & \textbf{0.104} \\
Weather & — & 0.195 & — & — & \textbf{0.061} \\
\midrule
$\Delta$ vs best & & & & & --43\% / --52\% / --69\% \\
\bottomrule
\end{tabular}
