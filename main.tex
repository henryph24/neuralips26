...
\begin{abstract}
Standard mixture-of-experts (MoE) routing on frozen time series foundation models (TSFMs) collapses catastrophically: routing entropy drops to $0.000\pm0.000$ on every backbone with internal Reversible Instance Normalization (RevIN) when layers are unfrozen. We diagnose the cause as \textbf{normalization-induced co-adaptation}---RevIN strips the per-window mean and scale that routers need to discriminate samples, while gradient feedback through unfrozen layers amplifies the collapse. A controlled ablation disabling RevIN inside MOMENT confirms that the collapse is causally attributable to instance normalization, and a cross-backbone control on Moirai~\citep{woo2024moirai} (which lacks RevIN) shows no collapse. We formalize this via an information-theoretic decomposition (Proposition~1): the mutual information lost by RevIN equals the routing information in precisely the temporal statistics it strips, predicting that datasets with higher location-scale variance should benefit more from raw routing---a prediction validated with Spearman $\rho{=}{-}0.96$ ($p{<}0.001$) across seven datasets, including correctly predicting the one dataset where RR-MoA does not help.
To break the collapse, we propose \textbf{Raw-Routed Mixture of Adapters (RR-MoA)}: a lightweight gate routes on the raw pre-normalization input while a strictly frozen backbone produces deep embeddings for the experts. RR-MoA achieves \textbf{54/54 wins} across 6 datasets, 3 freeze levels, and 3 seeds against single-adapter baselines ($p{<}0.001$, Wilcoxon), and significantly outperforms LoRA, TRACE-style adapters, and independent ensembles. On Moirai, RR-MoA matches DLinear~\citep{zeng2023dlinear} on Weather ($0.209$ vs $0.208$). We uncover a \textbf{Frozen Paradox}: strictly frozen routing outperforms partial unfreezing on 4/6 datasets, reversing the standard PEFT heuristic.
\end{abstract}
...
\textbf{The Frozen Paradox.} On ETTh1 and ETTm1, the \emph{strictly frozen} backbone yields RR-MoA's best absolute MSE; on Weather, last-2 is marginally better ($-47.2\%$ vs $-44.6\%$). Across all 6 datasets (Appendix~\ref{app:extended_rrmoa}), strictly frozen is the best configuration on 4 and within $13\%$ on the remaining 2, \emph{reversing} the standard NLP PEFT heuristic. The mechanism is Table~\ref{tab:adamix}: unfreezing the backbone is precisely what destroys routing diversity. We verify this paradox is not an artifact of low adapter capacity: repeating the ablation with the AAS-discovered macro expert pool ($271$K params, $9\times$ the canonical pool) shows frozen is still best on ETTh1 ($0.764{\pm}0.015$ vs $0.806{\pm}0.128$ for last-4), while ETTm1 and Weather show partial recovery under unfreezing ($0.510$ vs $0.583$ on ETTm1). The boundary is adapter capacity: low-capacity experts cannot exploit the additional gradient signal from unfreezing, while high-capacity experts can partially recover---but frozen routing remains competitive in all cases.
...
We identified \emph{normalization-induced co-adaptation} as the cause of MoE routing collapse on TSFMs, proved via three converging controls and formalized via Proposition~\ref{thm:mi_decomp}, whose quantitative prediction is validated at $\rho{=}{-}0.96$ ($p{<}0.001$) across seven datasets. \textbf{RR-MoA} breaks the collapse by routing on the raw pre-normalization signal with a frozen backbone (\textbf{54/54 wins}, $p{<}0.001$, three backbones, four horizons, two tasks). The \emph{Frozen Paradox}---frozen outperforms unfreezing---reverses the standard PEFT heuristic. More broadly, the normalization-routing tension applies to any frozen FM with internal normalization; routing on pre-normalization features is a general recommendation for PEFT + MoE.

\textbf{Limitations.} (1)~Absolute MSE gap to supervised baselines on MOMENT-small (narrows on Moirai). (2)~Decoder-only and native MoE backbones not evaluated. (3)~Frozen Paradox explanation is mechanistic, not a formal optimality proof.
