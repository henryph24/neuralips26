...
\textbf{Limitations.} We demonstrate cross-backbone generalization on two MOMENT variants (small and large) with Chronos and Moirai experiments in progress. The standard benchmark MSEs (${\sim}1.0$ on ETTh1) are higher than published numbers from full fine-tuning methods (${\sim}0.37$ for PatchTST), reflecting the frozen-backbone adapter paradigm rather than a limitation of the search method. Multi-seed evaluation is complete for ETTh1 (3 seeds) and ETTm1 (3 seeds); extending to all datasets is straightforward.
