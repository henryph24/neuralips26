...
\begin{enumerate}
    ...
    \item[\textup{(ii)}] If $(M, \Sigma) \perp\!\!\!\perp S$, then $I(X; E) - I(S; E) \geq I(M, \Sigma;\, E)$, with equality iff $(M,\Sigma) \perp\!\!\!\perp S \mid E$.
    \item[\textup{(iii)}] If additionally $E \perp\!\!\!\perp S \mid (M, \Sigma)$, then $I(X; E) - I(S; E) = H(E)$.
\end{enumerate}
\end{proposition}

Part~(i) is the DPI applied to $X \to S \to E$. Part~(ii) uses the chain rule: $I(X;E) = I(S;E) + I(M,\Sigma; E \mid S)$; under $(M,\Sigma) \perp\!\!\!\perp S$ this simplifies to $I(S;E) + I(M,\Sigma; E) + I(M,\Sigma; S \mid E)$, giving a lower bound since $I(M,\Sigma; S \mid E) \geq 0$. The residual term $I(M,\Sigma; S \mid E)$ captures the \emph{collider effect}: conditioning on the expert assignment $E$ can induce dependence between otherwise independent statistics and shape, meaning RevIN may destroy \emph{more} routing information than $I(M,\Sigma; E)$ alone. Part~(iii) shows that when routing depends \emph{entirely} on the stripped statistics, RevIN destroys \emph{all} routing information ($H(E)$ nats). Full proof in Appendix~\ref{app:proofs}. The independence $(M,\Sigma) \perp\!\!\!\perp S$ is RevIN's design assumption; in practice, shape and volatility may weakly correlate in real time series, but whatever mutual information leaks through $S$ is demonstrably insufficient to prevent collapse---routing entropy reaches exactly $0.000\pm0.000$ under RevIN (Table~\ref{tab:adamix}), confirming the bound is tight in the regime that matters. The conditional independence $E \perp\!\!\!\perp S \mid (M,\Sigma)$ is validated by Figure~\ref{fig:routing_viz}, where expert selection clusters by amplitude and volatility regardless of waveform shape.
...
\textbf{What the router learns.} Per-sample routing decisions correlate with amplitude and volatility---the very statistics RevIN strips (Figure~\ref{fig:routing_viz} in Appendix~\ref{app:routing_weights}). On ETTh1, Expert~0 (mean-pool) serves as the generalist ($53\%$), while Expert~2 (max-pool) handles low-amplitude windows and Expert~3 (attention-pool) handles high-amplitude windows. The router reserves specialized processing for windows at the distribution tails.
...
Top-2 sparse routing adds $4.3$\,ms ($+10\%$) latency over a single adapter (Appendix~\ref{app:topk},\ref{app:benchmark}). RR-MoA wins \textbf{24/24} horizon configurations ($H{\in}\{96,192,336,720\}$, $-38\%$ to $-51\%$); the DLinear gap narrows with $H$ ($+83\%{\to}+48\%$ on ETTh1, $+58\%{\to}+14\%$ on Weather). RR-MoA also generalizes to imputation ($-44\%$ to $-67\%$; Appendix~\ref{app:imputation}). An information diagnostic (Appendix~\ref{app:diagnostic}) confirms the residual gap reflects genuine information loss: k-NN on backbone features is $16$--$33\%$ worse than on raw input.

\section{Conclusion}

We identified \emph{normalization-induced co-adaptation} as the cause of MoE routing collapse on TSFMs, proved via three converging controls and formalized via Proposition~\ref{thm:mi_decomp} ($\rho{=}{-}0.96$, $p{<}0.001$). \textbf{RR-MoA} breaks the collapse by routing on raw pre-normalization signal with a frozen backbone (\textbf{54/54 wins}, three backbones, four horizons, two tasks). The \emph{Frozen Paradox}---frozen RR-MoA beats full fine-tuning by $12$--$79\%$---reverses standard PEFT. Collapse generalizes to BatchNorm1d/GroupNorm; a vision control (Appendix~\ref{app:vision_moe}) shows the normalization trigger is modality-specific, as Proposition~\ref{thm:mi_decomp} predicts.

\textbf{Limitations.} (1)~MSE gap to DLinear on MOMENT-small (narrows with horizon/backbone; dual-stream experts close it to $+9\%$ on ETTh1 and beat DLinear on Weather; Appendix~\ref{app:gap_closing}). (2)~Native MoE backbones (Moirai-MoE, Time-MoE) not evaluated. (3)~Proposition~\ref{thm:mi_decomp} linear only; nonlinear open. (4)~Vision control: collapse trigger is modality-specific. (5)~Expert pool hand-selected; cross-channel routing unexplored.
...
\end{align*}
Under the assumption $(M, \Sigma) \perp\!\!\!\perp S$, we expand:
\begin{align*}
I(M, \Sigma;\, E \mid S) &= H(M, \Sigma \mid S) - H(M, \Sigma \mid E, S) \\
&= H(M, \Sigma) - H(M, \Sigma \mid E, S) = I(M, \Sigma;\, E, S) \\
&= I(M, \Sigma;\, E) + \underbrace{I(M, \Sigma;\, S \mid E)}_{\geq\, 0}
\end{align*}
The non-negative residual $I(M,\Sigma; S \mid E)$ arises because $E$, as a function of $(M,\Sigma,S)$, acts as a collider: conditioning on $E$ can induce dependence between its otherwise independent parents $(M,\Sigma)$ and $S$ (Berkson's paradox). Therefore $I(X;E) - I(S;E) = I(M,\Sigma;\,E) + I(M,\Sigma;\,S \mid E) \geq I(M,\Sigma;\,E)$, with equality iff $(M,\Sigma) \perp\!\!\!\perp S \mid E$.
...
\begin{figure}[h]
\centering
\includegraphics[width=\columnwidth]{figures/routing_viz.pdf}
\caption{\textbf{Per-sample expert assignment vs.\ input statistics} (ETTh1, seed 42, $N{=}5{,}000$ test windows). Left: amplitude vs.\ volatility; right: mean level vs.\ amplitude. Colors indicate dominant expert (top-1 routing weight). Expert selection clusters by amplitude and volatility---the very statistics RevIN strips---validating the independence assumption $(M,\Sigma) \perp\!\!\!\perp S$ from Proposition~\ref{thm:mi_decomp}.}
