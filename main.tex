...
\textbf{The time-series discretization gap.} Standard continuous NAS fails on frozen TSFMs: DARTS~\citep{liu2019darts} achieves 1/6 wins after discretization (co-adapted weights break), and hidden-state MoE routing collapses (1/18 wins, 6\%). The root cause: TSFM LayerNorm homogenizes hidden states, destroying the per-sample heterogeneity needed for routing. This is \textit{specific to frozen TSFMs}---vision transformers retain token heterogeneity. RR-MoA resolves this by routing on the raw input (17/18 wins), proving the diagnosis is correct and the fix is sound.
