...
\item \textbf{Consistent empirical improvement}: evolved adapters beat hand-designed baselines on \textbf{all 17 experiments} across 5 datasets (ETTh1, ETTh2, ETTm1, ETTm2, Weather), 3 random seeds, 2 forecast horizons, and 2 backbone sizes (MOMENT-small and MOMENT-large), with an average improvement of 11.1\%.
...
Tables~\ref{tab:main} and~\ref{tab:backbone} show results on the held-out test set. Evolved adapters---whose architecture is discovered via search on the validation set---beat the best fixed adapter on \textbf{all 17 experiments}.
...
\midrule
\multicolumn{5}{l}{\textit{Moirai-1.1-R-small$^\dagger$ (14M params, $d_\text{model}{=}384$, 6 blocks)}} \\
& ETTh1 & — & — & — \\
\bottomrule
\end{tabular}
\end{table}
...
\textbf{Limitations.} We demonstrate cross-backbone generalization on two MOMENT variants (small and large) with Chronos and Moirai experiments in progress. The standard benchmark MSEs (${\sim}1.0$ on ETTh1) are higher than published numbers from full fine-tuning methods (${\sim}0.37$ for PatchTST~\cite{nie2023patchtst}), reflecting the frozen-backbone adapter paradigm rather than a limitation of the search method. Multi-seed evaluation is complete for ETTh1 (3 seeds) and ETTm1 (3 seeds); extending to all datasets is straightforward.

\section{Conclusion}

We presented Code Evolution, a framework for searching the unbounded space of PyTorch adapter modules for time series foundation model adaptation. Task-specific architecture search discovers adapter designs---including depthwise convolutions, learned feature attention, and BatchNorm gating---that beat hand-designed baselines on \textbf{17/17 experiments} across 5 datasets, 3 seeds, 2 horizons, and 2 backbone sizes (MOMENT-small and MOMENT-large), with 11.1\% average MSE improvement. The framework completes in 30 minutes on a single GPU and reveals that the shift from discrete configurations to code-level search is the dominant driver of improvement ($p{=}0.031$). We hope this work encourages the community to look beyond hyperparameter tuning toward automated, code-level adapter design for foundation models across modalities.
